...
public static class BookingConsole
{
	...
	{
		Console.Write(
			!string.IsNullOrEmpty(currentValue) ? $"{prompt} [{currentValue}]: " : $"{prompt}: "
		);
		string input = Console.ReadLine() ?? "";
	...
	public static void ShowBookings(List<Booking> bookings)
	{
		...
		Console.WriteLine(
			$"  {"ID", -5} {"Navn", -20} {"Rom", -8} {"Dato", -12} {"Start", -7} {"Slutt", -7} Beskrivelse"
		);
		...
		{
			Console.WriteLine(
				$"  {b.Id, -5} {b.CustomerName, -20} {b.Room, -8} {b.Date, -12} {b.Start, -7} {b.End, -7} {b.Description}"
			);
		}
		Console.WriteLine();
	}
